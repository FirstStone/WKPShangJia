...
@interface EditEmployeeViewController ()<UITableViewDelegate,UITableViewDataSource,InPutViewControllerDelegate,WKPPickerViewControllerDelegate>
...
- (void)setUpUI
{
    if (self.employee) {
            ...
            [_dict setObject:self.employee.jobid forKey:@"id"];
    }

    _dataArry = [[NSMutableArray alloc]initWithArray:@[self.employee.realname,self.employee.mobile,self.employee.jobName]];
...
-(void)addEmployee
{
    ...
    [param setObject:[defaults objectForKey:@"shopid"] forKey:@"shopid"];
    [param setObject:[defaults objectForKey:@"mid"] forKey:@"mid"];
    [param setObject:self.employee.userid forKey:@"editUserID"];
    ...
    [WKPHttpRequest post:WKPEditShopMember param:param finish:^(NSData *data, NSDictionary *obj, NSError *error) {
...
-(UIView *)setFooterView
{
    ...
    UILabel * lbl1  = [[UILabel alloc]initWithFrame:CGRectMake(lbl.frame.origin.x, CGRectGetMaxY(lbl.frame) , SCREEN_WIDTH - 20, 60)];

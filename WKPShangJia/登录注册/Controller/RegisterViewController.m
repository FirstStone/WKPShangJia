...
- (void)setUpUI
{
    ...
    [btn addTarget:self action:@selector(wkpRegister) forControlEvents:UIControlEventTouchUpInside];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 2) {
        ...
        UIView * lineView = [[UIView alloc]initWithFrame:CGRectMake(_passWordLbl.frame.origin.x, CGRectGetMaxY(_passWordLbl.frame) + 9, + SCREEN_WIDTH  - _passWordLbl.frame.origin.x * 2 ,1)];
        lineView.backgroundColor = WKPColor(244, 244, 244);
...
- (void)getCode
{
    ...
    [param setObject:@(1) forKey:@"smstype"];
...
- (void)wkpRegister
{
    ...
    [param setObject:@(2) forKey:@"identity"];
    [param setObject:@(1) forKey:@"source"];
    [WKPHttpRequest post:WKPRegister param:param finish:^(NSData *data, NSDictionary *obj, NSError *error) {
        if([[obj objectForKey:@"msg"] isEqualToString:@"注册成功"]){
            [SVProgressHUD showSuccessWithStatus:[obj objectForKey:@"msg"]];
            [SVProgressHUD dismissWithDelay:1.0f];
            SubmitQualificationViewController * submitQualificationVC =[[SubmitQualificationViewController alloc]init];
            submitQualificationVC.registerMid = [obj objectForKey:@"mid"];
